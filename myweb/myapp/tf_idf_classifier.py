...
def tf_idf():
    ...
    test_title = list(test_t) #test news titlw
    for i in test_verb:
        test.append(i.stemmedVerb)

    test_data1 = list(test) #test_data = test verbs of test new
...
# tf_idf_neutral[row] =  tf_idf_nu
    ...
    for i in range(len(test_news)):
        if (variable1 > 0 ) or (variable2 > 0 ) or (variable3 > 0):
            ...
            if (tf_idf_positive[i] > tf_idf_negative[i]) and (tf_idf_positive[i] > tf_idf_neutral[i]):
                positive_news.append(test_title[i])
                r = tfidfPositiveClassified(s_no = p[i],title =  t[i],description =des[i],article = a[i],url = u[i],image_url  = iu[i],pubDate = pd[i])
                r.save()
               

            elif (tf_idf_negative[i] > tf_idf_neutral[i]) and (tf_idf_negative[i] > tf_idf_positive[i]):
                negative_news.append(test_title[i])
                r = tfidfNegativeClassified(s_no = p[i],title =  t[i],description =des[i],article = a[i],url = u[i],image_url  = iu[i],pubDate = pd[i])
                r.save()
               

            elif (tf_idf_neutral[i] > tf_idf_negative[i]) and (tf_idf_neutral[i] > tf_idf_positive[i]):
                neutral_news.append(test_title[i])
                r = tfidfNeutralClassified(s_no = p[i],title =  t[i],description =des[i],article = a[i],url = u[i],image_url  = iu[i],pubDate = pd[i])
                r.save()
               

            else:
                error_data.append(test_title[i])
